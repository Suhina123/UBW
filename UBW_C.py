...
# /content/drive/MyDrive/Colab Notebooks/UBW-C/model.py
...
CLASS_NUM = 50
BETA = 2.0
downsample_size = (32, 32)  # Change this to the desired size

print ("Source_class", type(SOURCE_CLASS))
...
def prepare_datasets(source_class, target_class):
    if DATASET == 'CIFAR10':
        ...
        transforms.Resize(downsample_size),  # Add this line to downsample the image
        transforms.RandomCrop(downsample_size, padding=5),
        ...
        trainset = torchvision.datasets.CIFAR10(
            root='/dockerdata/mavisbai', train=True, download=True, transform= transform_train,
        )
        testset = torchvision.datasets.CIFAR10(
            root='/dockerdata/mavisbai', train=False, download=True, transform= transform_test,
        )
        class_number = 10
    elif DATASET == 'GTSRB':
        ...
        trainset = torchvision.datasets.DatasetFolder(
            root='C:/dockerdata/mavisbai/GTSRB/train', # please replace this with path to your training set
            loader=cv2.imread,
            extensions=('png',),
        ...
        testset = torchvision.datasets.DatasetFolder(
            root='C:/dockerdata/mavisbai/GTSRB/testset', # please replace this with path to your test set
            loader=cv2.imread,
            extensions=('png',),
        ...
        testset = {x: torchvision.datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x]) for x in ['val']}
        class_number = 200
    # print("Trainset(train)", type(trainset))
    # # for data, label in trainset['train']:
    # #     print(data,label)

    # print("Trinset(val)", testset['val'])
    # for data, label in testset['val']:
    #     print(data,label)
    # print("Testset(test)", testset['val'])
    #data_sample = trainset['train'][0] 


    # print("Data Sample1", data_sample)
    # print("Length of tarinset['train']",  len(trainset['train'][0]))
    # print("Length of trainset[train][0][0]", len(trainset['train'][0][0]))
    # print("Length of trainset[train][0][1]", len(trainset['train'][0][1]))
    # print("Length of trainset[train][0][2]", len(trainset['train'][0][2]))
  
    # print("tain[1]", trainset['train'][1])
    # print("\n\n")
    # print("Length of trainset[train][0][0][0]", len(trainset['train'][0][0][0]))
    
    # data_sample = trainset['train'][1] 
    # print("Data Sample2", data_sample)

    #print("Shape of tensor", trainset.shape)

    print("TRainset", trainset)
    print("Testset", testset)

    # if class_number != CLASS_NUM:
    #     # trainset = [data for data in trainset['train']  if torch.any((data[1] >= 0) & (data[1] < CLASS_NUM))]
    #     # testset = [data for data in testset['val']  if torch.any((data[1] >= 0) & (data[1] < CLASS_NUM))]
...
def get_model():
    #if DATASET == 'CIFAR10' or DATASET == 'GTSRB':               ##chnaged CLASS_NUM and commented line 479,480
    mod = ResNet18(10).to(device)
